...
def simulate_remaining_season(league, models, current_week, n_sims=10000, lineup_override=None,
    ...
    Also returns player_weekly_means: {player_name: {week: adjusted_mean}}
    — the exact per-week expected value used inside this simulation for
    every rostered player. This is what season-total projections should
    be built from (summed across weeks) rather than a flat multiply, so
    the displayed season total is actually consistent with what the
    simulation itself is using.
    """
    ...
    for wk in remaining_weeks:
        ...
        for m in matchups:
            ...
            def build_side(team_id, lineup):
                if team_id in lineup_override:
                    return [models.get(n, (0, 4)) for n in lineup_override[team_id]]
                dists = []
                for bp in lineup:
                    if bp.lineupSlot in ("BE", "IR"):
                        continue
                    dist = matchup_adjusted_dist(
                        models, defense_vs_position, league_avg_allowed,
                        bp.name, bp.position, getattr(bp, "pro_opponent", None),
                    )
                    dists.append(dist)
                    player_weekly_means.setdefault(bp.name, {})[wk] = dist[0]
                return dists

            home_dists = build_side(m.home_team.team_id, m.home_lineup)
            away_dists = build_side(m.away_team.team_id, m.away_lineup)
    ...
    return avg_wins, playoff_odds, player_weekly_means
...
def start_sit_for_my_team(league, models, defense_vs_position, league_avg_allowed, player_scores, player_weekly_means, current_week):
    ...
    for p in my_team.roster:
        ...
        # Season total = points already scored + the sum of each remaining
        # week's actual matchup-adjusted expected value used by the
        # simulation (not a flat season-average multiply).
        played_total = sum(player_scores.get(p.name, []))
        future_means = player_weekly_means.get(p.name, {})
        season_total = round(played_total + sum(future_means.values()), 1)
...
def render_html(league, current_week, avg_wins, playoff_odds, tiers, my_team, my_recs, this_week=None):
...
<table><tr><th>Team</th><th>Proj. Wins</th><th>Playoff Odds</th></tr>"""]
    ...
    if my_team:
        ...
        html.append(
            ...
            "allowed to that position this season (higher = more favorable matchup). "
            "Proj. Season Total sums each remaining week's actual matchup-adjusted "
            "projection — the same numbers the simulation uses, not a flat average.</p>"
        )
        html.append(
    ...
    html.append(
        ...
        '<p class="note">Top QBs, RBs, WRs &amp; TEs on the wire, ranked by projected value. '
        "Season total here is a straight-line estimate (season pace), since we don't have "
        "reliable per-week opponent data for players not on any roster.</p>"
    )
...
def main():
    ...
    avg_wins, playoff_odds, player_weekly_means = simulate_remaining_season(
    ...
    my_team, my_recs = start_sit_for_my_team(
        league, models, defense_vs_position, league_avg_allowed,
        player_scores, player_weekly_means, current_week,
    )
    ...
    html = render_html(league, current_week, avg_wins, playoff_odds, tiers, my_team, my_recs, this_week)
