...
def build_player_models(league, current_week, extra_players=None):
    ...
    Returns (models, player_scores) — player_scores is each player's
    raw per-week actual point history, reused by the one-week-add
    logic below to spot recent usage spikes (e.g. a backup stepping up
    after an injury) using real results rather than a forward
    projection ESPN doesn't reliably expose for free agents.
    """
    player_scores = {}
...
def simulate_this_week(league, models, my_team_id, current_week, n_sims=10000):
    ...
    wins = 0
    for _ in range(n_sims):
        home_score = sum(random.gauss(*models.get(p.name, (0, 4))) for p in get_starting_lineup(home_team))
        away_score = sum(random.gauss(*models.get(p.name, (0, 4))) for p in get_starting_lineup(away_team))
...
def one_week_add_candidates(free_agents, models, player_scores, current_week, top_n=8, recent_window=2, min_recent_avg=4.0):
    """
    ...
    for p in free_agents:
        ...
        if spike > 0 and recent_avg >= min_recent_avg:
            candidates.append((p, round(recent_avg, 1), round(spike, 1)))
...
def render_html(league, current_week, avg_wins, playoff_odds, tiers, streamers, my_team, my_recs, this_week=None):
    ...
    if top_streamer:
        p, recent_avg, spike = top_streamer
...
<table><tr><th>Team</th><th>Proj. Wins</th><th>Playoff Odds</th></tr>"""]
    ...
    html.append(
        ...
        "<table><tr><th>Player</th><th>Position</th><th>Recent Avg</th><th>Spike vs. Season Avg</th></tr>"
    )
    for p, recent_avg, spike in streamers:
        html.append(f"<tr><td>{p.name}</td><td>{p.position}</td><td>{recent_avg}</td><td>+{spike}</td></tr>")
    html.append("</table></div>")
...
def main():
    ...
    models, player_scores = build_player_models(league, current_week, extra_players=free_agents)
    ...
    streamers = one_week_add_candidates(free_agents, models, player_scores, current_week)
    ...
    summary = {
        ...
        "one_week_adds": [{"name": p.name, "recent_avg": ra, "spike": sp} for p, ra, sp in streamers],
        "this_week": this_week,
    }
